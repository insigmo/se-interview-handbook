...
<details> <summary><strong>Вопрос:</strong> Решите задачу Two Sum (найти индексы двух чисел, дающих target).</summary>

**Ответ:**  
Использовать хэш-мапу `value -> index`: идём по массиву, для каждого `x` проверяем, есть ли `target - x` в мапе; если да — возвращаем индексы, если нет — кладём `x` в мапу.

</details><details> <summary><strong>Вопрос:</strong> Что такое каналы (channels) в Go и как они используются?</summary>
**Ответ:**  
Канал — типизированный поток для безопасной передачи данных между горутинами; операции `send/receive` синхронизируют выполнение, можно делать буферизированные/небуферизированные каналы, закрывать канал, читать через `range`.

</details><details> <summary><strong>Вопрос:</strong> Что такое slice в Go и чем он отличается от массива?</summary>

**Ответ:**  
Slice — “view” на массив: содержит указатель на массив, `len` и `cap`; динамически изменяемый (через `append`), массив — фиксированного размера и является значением.

</details><details> <summary><strong>Вопрос:</strong> Что такое map в Go и какие есть особенности?</summary>

**Ответ:**  
`map` — хэш-таблица `key -> value`; порядок не гарантирован, чтение несуществующего ключа даёт нулевое значение типа, запись/чтение из `nil map` отличается (читать можно, писать нельзя), конкурентная запись без синхронизации запрещена.

</details>
<details> <summary><strong>Вопрос:</strong> Спроектируйте сервис сокращения ссылок: какие эндпоинты и что они делают?</summary>

**Ответ:**

- `POST /shorten` принимает длинный URL и возвращает `short_code`.
    
- `GET /{code}` делает редирект на исходный URL.
    
- `GET /stats/{code}` возвращает исходный URL и число кликов.
    

</details><details> <summary><strong>Вопрос:</strong> Как генерировать короткий код, чтобы было мало коллизий?</summary>

**Ответ:**  
Варианты: (1) автоинкрементный ID + Base62; (2) случайная строка Base62 достаточной длины с проверкой коллизии в БД; (3) Snowflake/ULID → Base62.

</details><details> <summary><strong>Вопрос:</strong> Где хранить соответствие <code>code → url</code> и счётчик кликов?</summary>

**Ответ:**  
В БД (PostgreSQL/Redis/Key-Value). Для кликов: либо атомарный инкремент в Redis и периодический сброс в БД, либо сразу в БД (но дороже по нагрузке).

</details><details> <summary><strong>Вопрос:</strong> Как правильно делать редирект в <code>GET /{code}</code>?</summary>

**Ответ:**  
Возвращать HTTP 301/302/307/308 с заголовком `Location: original_url`. Обычно 302/307 для динамики, 301/308 для постоянных (зависит от требований к кешированию).

</details>
<details> <summary><strong>Вопрос:</strong> Что делать, если заметил брутфорс пароля камеры (или любого сервиса)?</summary>

**Ответ:**  
Ввести rate limiting (по IP/аккаунту/устройству), добавить backoff/бан, капчу/2FA где уместно, логирование и алерты, блок-листы, ограничение по гео/сетям, проверка на утечки, смена пароля/отключение уязвимого доступа.

</details><details> <summary><strong>Вопрос:</strong> Реализуйте rate limiter: какой подход “правильный” на практике?</summary>

**Ответ:**  
Классика: token bucket / leaky bucket. Для распределённого сервиса — хранить счётчики/токены в Redis (Lua/atomic ops), учитывать ключ (IP, userID, endpoint), выставлять TTL.

</details><details> <summary><strong>Вопрос:</strong> В код-ревью: читаем из канала, но его не закрывают. В чём проблема и как правильно?</summary>

**Ответ:**  
Если потребитель читает через `range ch`, то без `close(ch)` он может ждать вечно и словить дедлок/утечку горутин. Закрывать должен **тот, кто пишет** (producer) после завершения отправки; альтернативно — использовать `context`/done-канал для остановки.

</details><details> <summary><strong>Вопрос:</strong> Найдите уязвимость: в БД собирают запрос через строку с <code>%</code> / конкатенацией. Что не так?</summary>

**Ответ:**  
Это риск SQL-инъекции. Нужно использовать параметризованные запросы (placeholders), подготовленные выражения, query builder/ORM, валидировать входные данные и не подставлять пользовательский ввод в SQL-строку напрямую.

</details>
<details> <summary><strong>Вопрос:</strong> Что спрашивали на финальном этапе в “обычную” команду?</summary>

**Ответ:**  
Разговор про опыт: чем занимался, какие проекты, ответственность, подход к разработке/взаимодействию в команде, ожидания.
